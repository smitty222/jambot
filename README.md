...
For user-scoped Spotify/queue commands, each user needs their own TT Live token stored as an environment variable (e.g. `USERNAME_USER_TOKEN`). These should be treated as secrets and must not be hardcoded in source control.

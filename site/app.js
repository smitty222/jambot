...
async function saveTokenHandler () {
  ...
  try { await refreshMod(); setTokenStatus(val ? 'Saved. If mod still says Unauthorized, double-check the token.' : 'Cleared.', true) } catch { setTokenStatus('Saved, but error loading mod data.', false) }
}
...
async function refreshMod () {
  try {
    ...
    if (els.modCmds) {
      els.modCmds.innerHTML = (!mod
        ? "<div class='muted small'>Unauthorized</div>"

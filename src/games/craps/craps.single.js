...
async function renderLineResult (res) {
  ...
  const passWinSum = sumMap(passWinners); const passLoseSum = sumMap(passLosers); const dpWinSum = sumMap(dpWinners)
  const dpLoseSum = sumMap(dpLosers); const dpPushSum = sumMap(dpPushes)
...
- Players seated: **${players}**`
  ...
  if ((m = txt.match(/^\/pass\s+(\S+)/i))) {
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'NOT_ALLOWED_NOW'
          ? 'Pass can only be placed during the come-out betting window.'
  ...
  if ((m = txt.match(/^\/dontpass\s+(\S+)/i))) {
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'NOT_ALLOWED_NOW'
          ? 'Don’t Pass can only be placed during the come-out betting window.'
          : res.reason === 'INSUFFICIENT_FUNDS'
            ? 'Insufficient funds.'
  ...
  if ((m = txt.match(/^\/place\s+(\d+)\s+(\S+)/i))) {
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'NOT_ALLOWED_NOW'
          ? 'Place bets are only during the point betting window.'
          : res.reason === 'BAD_NUMBER'
            ? 'Number must be 4, 5, 6, 8, 9, or 10.'
            : res.reason === 'INSUFFICIENT_FUNDS'
              ? 'Insufficient funds.'
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'BAD_NUMBER'
          ? 'Number must be 4, 5, 6, 8, 9, or 10.'
          : res.reason === 'NONE'
            ? 'You have no Place bet on that number.'
  ...
  if ((m = txt.match(/^\/field\s+(\S+)/i))) {
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'NOT_ALLOWED_NOW'
          ? 'Field can only be placed during the point betting window while Bets are OPEN.'
          : res.reason === 'INSUFFICIENT_FUNDS'
            ? 'Insufficient funds.'
  ...
  if ((m = txt.match(/^\/double\s+(\d)\s+(\S+)/i))) {
    ...
    if (!res.ok) {
      const msg =
        res.reason === 'NOT_ALLOWED_NOW'
          ? 'Doubles can only be placed during the point betting window while Bets are OPEN.'
          : res.reason === 'BAD_PIP'
            ? 'Pip must be 1–6.'
            : res.reason === 'INSUFFICIENT_FUNDS'
              ? 'Insufficient funds.'

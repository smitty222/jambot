...
async function joinTable(userUUID, nickname) {
    ...
    if (!canJoinTable) {
        await postMessage({ room: room, message: `${nickname}, the game has already started. Please wait until the round ends` });
    ...
    if (!tableUsers.includes(userUUID)) {
        ...
        await postMessage({ room: room, message: `${nickname} has joined the blackjack table.` });
    } else {
        await postMessage({ room: room, message: `${nickname} is already at the table.` });
    }
}
...
async function leaveTable(userUUID) {
    ...
    if (index !== -1) {
        ...
        await postMessage({ room: room, message: `User ${userUUID} has left the blackjack table.` });
    } else {
        await postMessage({ room: room, message: `User ${userUUID} is not at the table.` });
    }
}
...
async function startGame() {
    ...
    await postMessage({ room: room, message: `Dealer's hand: ${dealerHand[0].value}${dealerHand[0].suit} ?` });
...
async function promptPlayerTurn() {
    ...
    await postMessage({ room: room, message: `It's ${currentPlayerNickname}'s turn. Choose /hit or /stand.` });
}
...
async function handleHit(userUUID, nickname) {
    ...
    if (tableUsers[currentPlayerIndex] !== userUUID) {
        await postMessage({ room: room, message: `It's not your turn, ${nickname}.` });
        return;
    }
    ...
    await postMessage({ room: room, message: `${nickname} hits: ${newCard.value}${newCard.suit}. Total value: ${playerValue}` });

    // Check if the player's hand value is greater than 21 (bust)
    if (playerValue > 21) {
        await postMessage({ room: room, message: `${nickname} busted!` });
        await handleNextPlayer(); // Move to the next player
    } 
    // Check if the player's hand value is exactly 21
    else if (playerValue === 21) {
        await postMessage({ room: room, message: `${nickname}, your hand value is 21! You stand automatically.` });
        await handleStand(userUUID, nickname); 
    }
    else {
        await postMessage({ room: room, message: `${nickname}, your hand value is ${playerValue}. Do you want to /hit or /stand?` });
    }
}
...
async function handleStand(userUUID, nickname) {
    ...
    await postMessage({ room: room, message: `${nickname} stands with value ${calculateHandValue(playerHands[userUUID])}.` });

...
import { decoratedMention, syncAlbumPlaysPrestige, syncDJDebutPrestige, formatPrestigeUnlockLines } from '../database/dbprestige.js'
...
const handleAlbumTheme = async (_payload) => {
  ...
  try {
    ...
    if (isFirst) {
      ...
      if (currentDJUuid) {
        const debutPrestige = syncDJDebutPrestige(currentDJUuid)
        const debutLines = formatPrestigeUnlockLines(debutPrestige)
        if (debutLines.length) await postMessage({ room, message: `<@uid:${currentDJUuid}>\n${debutLines.join('\n')}` })
      }

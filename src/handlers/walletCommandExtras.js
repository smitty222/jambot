...
export function formatBankrollLine ({ rank, uuid, name, amount, mentionUser = decoratedMention }) {
  const userLabel = uuid ? mentionUser(uuid) : compactLeaderboardName(name, uuid)
  const numeric = Number(amount || 0)
  const money = `${numeric < 0 ? '-' : ''}$${Math.round(Math.abs(numeric)).toLocaleString()}`
  return `${rank}. ${userLabel} ${money}`
}
...
export async function handleBankrollCommand ({ room }) {
  try {
    ...
    const sortedBankroll = bankroll
      ...
      .slice(0, 5)
      .map((user, index) => formatBankrollLine({
        rank: index + 1,
        uuid: user.uuid,
        name: user.nickname,
        amount: user.balance
